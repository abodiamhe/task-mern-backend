...
test('should set a new task for a user', async () => {
  const req = {
    user: { id: 'user-id' },
    body: { text: 'Learn testing javascript' },
  };
  const res = {
    status: jest.fn().mockReturnThis(),
    json: jest.fn(),
  ...
  //Mocking the create task
  const task = { _id: 'task-id-3', text: 'Task 3', user: 'user-id' };
...
//testing update controller for user not found
test('should return a 401 error if user is not found', async () => {
  ...
  //Mocking the findById to return the task to be updated
...
test('should return a 401 error if user is not authorized to update the task', async () => {
  ...
  const req = {
    params: { id: 'taskId' },
    user: { id: userId },
    body: { text: 'Updated tTask' },
  };

  //Mocking the task to be updated, owned by a different user
  const taskUpdate = { _id: taskId, text: 'Original Task', user: 'user-id-1' };

  //Mocking the findById method to return the user
  Task.findById.mockResolvedValue(taskUpdate);
